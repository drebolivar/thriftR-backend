...
function randAggregation(options) {
  ...
  const values = (_options$values = options == null ? void 0 : options.values) != null ? _options$values : 2;
  const totalValue = (_options$totalValue = options == null ? void 0 : options.totalValue) != null ? _options$totalValue : 100;
  const noZeros = (_options$noZeros = options == null ? void 0 : options.noZeros) != null ? _options$noZeros : false;
  ...
  return fake(() => {
    ...
    nums[nums.length - 1] += totalValue - nums.reduce((a, b) => a + b, 0); // Shuffle the array

    const shuffled = []; // nums is guaranteed to have a length over 1 so this can't cause an infinite loop
